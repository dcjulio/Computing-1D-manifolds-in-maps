...
%   Manif = GrowFundCurv1D(opts) computes the one-dimensional invariant manifold
%   of a designated fixed point for a 3D discrete dynamical system.
%
%   INPUTS:
%       - opts : Structure containing system parameters, target arclength,
%                max iterations, and accuracy parameters.
%       - eigenval_idx : (Optional) Integer [1 or 2] selecting which 
%                        eigenvector to choose to compute an orbit in a higher-dimensional (2D) manifold.
%
%   OUTPUTS:
%       - Manif : Structure storing the computed manifold mesh
%                 points, arclengths, preimage mapping indices, and runtime
%                 diagnostics.
%% INPUT VALIDATION & MANIFOLD INITIALIZATION
if nargin < 2
    eigenval_idx = [];
else
...
% Allocate and pre-fill the manifold structure using user-defined choices
manif = init_manif(opts);

tic % Start execution timer for performance metrics

% Extract core parameter values and the user-defined system class
par=manif.system_info.par;
thesystem=opts.thesystem;

%% ORIENTABILITY & STABILITY CONFIGURATION

% Comment on screen: Orientation-reversing manifolds flip across the fixed point at each iteration,
% hence, the computation stores both branches.
if strcmp(manif.orientability,'orientation-reversing')
    fprintf('\n----  Manifold is non-orientable. We will proceed computing both branches\n');
    mapiter=2; % First "return" of the manifold occurs on iteration 2
else
    mapiter=1; % First "return" of the manifold occurs right away (on iteration 1)
end

% Determining mapping direction: Forward mapping for Unstable manifolds ($W^u$),
% backward mapping (inverse) for Stable manifolds ($W^s$)
if strcmp(manif.stability,'Smanifold')
    sign = -1; % Use inverse map ff_inv
elseif strcmp(manif.stability,'Umanifold')
    sign = 1; % Use forward map ff
end

% Establishing the names of primary and secondary branches based on init_step sign
...
%% HIGHER-DIMENSIONAL SUBSPACE HANDLING (MANIFOLD DIMENSION > 1)
% This toolkit is optimized for 1D manifolds. If the fixed point 
% has a higher dimensional manifolds (2D), the algorithm could anyway
% compute a submanifold (1D) along a chosen real eigenvector on its 2D
% manifold.
...
if manif.grow_info.dimension>1
    ...
    % Prompt the user for a selection if it wasn't provided as a function argument
    ...
    if x == 0
        ...
        % Isolate the selected eigenvalue and its corresponding vector
...
%% DIAGNOSTIC PRINTING (RUN CONFIGURATION)
    ...
    if strcmp(manif.orientability,'orientation-preserving')
        fprintf('\n----  Computing W%s(%s), %s branch up to arclength %.0f ---- ',lower(opts.stability(1)),opts.name_fixpoint,manif.name(end-2:end), opts.target_arc);
    else
        fprintf('\n----  Computing W%s(%s), both branches up to arclength %.0f each ---- ',lower(opts.stability(1)),opts.name_fixpoint,opts.target_arc);
    end
    ...
    %-------------------------------------------------------------------
    %-------------------------------------------------------------------
    % Initialize performance counters inside the runinfo structure
    manif.grow_info.runinfo.rem_deltamin=0;     % Points dropped due to overcrowding (< deltamin)
    manif.grow_info.runinfo.rem_nan=0;          % Points dropped due to numerical NaNs
    manif.grow_info.runinfo.rem_inf=0;          % Points dropped at the boundary of compactified infinity
    manif.grow_info.runinfo.add_alphamax=0;     % Points added via alpha_max
    manif.grow_info.runinfo.add_deltamax=0;     % Points added via delta_max
    manif.grow_info.runinfo.add_deltalphamax=0; % Points added via curvature condition (delta alpha)_max

%% GENERATE THE FIRST FUNDAMENTAL DOMAIN
...
while iter < manif.grow_info.max_funditer && stop_arc ~= 1 && (stop_arc_branch2 + stop_arc_branch1) ~= 2
    ...
    Manif.grow_info.runinfo.rem_nan=Manif.grow_info.runinfo.rem_nan+numel(nan_idx);
    ...
    Manif.grow_info.runinfo.rem_inf=Manif.grow_info.runinfo.rem_inf+numel(inf_idx);
    ...
    if strcmp(manif.orientability,'orientation-preserving')
        needed_arc = opts.target_arc - total_arc;
    elseif strcmp(manif.orientability,'orientation-reversing')
        if mod(iter,2)==0 
            needed_arc = opts.target_arc - total_arc_branch1;
        else
            needed_arc = opts.target_arc - total_arc_branch2;
        end
    end
...
%-----------%------ If only Delta0*alpha or Delta2*alpha > Delta*alphamax and the angle is ok (previous if is when alpha > alphamax or both delta0alpha and delta2alpha > deltaalphamax)
            ...
            if any(add_acc.add_new ~= 0)
                ...
                Manif.grow_info.runinfo.add_deltamax = Manif.grow_info.runinfo.add_deltamax + sum(add_acc.add_new == 1 | add_acc.add_new == 2);
                Manif.grow_info.runinfo.add_alphamax = Manif.grow_info.runinfo.add_alphamax + sum(add_acc.add_new == 3);
                Manif.grow_info.runinfo.add_deltalphamax = Manif.grow_info.runinfo.add_deltalphamax + sum(add_acc.add_new == 4 | add_acc.add_new == 5);
...
%---%----------- Final section: save info
    ...
    elseif strcmp(manif.orientability,'orientation-reversing')
        if mod(iter,2)==0
            ...
            %Check once again if we reached the desired arclength
            if total_arc_branch1 > opts.target_arc 
                stop_arc_branch1 = 1;
            end
            ...
            %Check once again if we reached the desired arclength
            if total_arc_branch2 > opts.target_arc 
                stop_arc_branch2 = 1;
            end
...
Manif.grow_info.runinfo.num_funditer=iter;
Manif.grow_info.runinfo.total_arclength=arc;
Manif.grow_info.runinfo.return_dist = return_dist;
Manif.grow_info.runinfo.time=toc;

fprintf('\n elapsed time is %.3f seconds\n\n', Manif.grow_info.runinfo.time)
if strcmp(manif.orientability,'orientation-preserving')
...
fprintf('   * %i points removed \n',Manif.grow_info.runinfo.rem_deltamin+Manif.grow_info.runinfo.rem_nan+Manif.grow_info.runinfo.rem_inf) 
fprintf('   * %i points added from deltamax \n',Manif.grow_info.runinfo.add_deltamax) 
fprintf('   * %i points added from alpha \n',Manif.grow_info.runinfo.add_alphamax) 
fprintf('   * %i points added from delta*alpha \n\n',Manif.grow_info.runinfo.add_deltalphamax) 
